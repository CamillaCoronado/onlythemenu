...
- **The image adapter works, but a scanned pdf can't reach it.** OCR was verified end to end against a rendered menu (6/6 items, exact names and prices, ~1s); tesseract fetches its language data fine with network. Two gaps remain: the pdf adapter throws `route to image adapter` when a pdf has no text layer but nothing performs that reroute, and tesseract needs a raster, so closing it means rendering pdf pages to images (a new dependency — `pdfjs-dist` alone won't rasterize in node). Cafe Sabor's menu is exactly this case. The image adapter also put a clear section heading under `menu` rather than using it as the title.
- **The image adapter still has no fixtures**, so nothing guards the above in CI.
...
- **Overture import** needs `--release <name>` and wasn't run here (no network). Check the category list against the release's taxonomy. It is also the only sane way to add restaurants in bulk: guessing domains from a restaurant's name lands on parked and squatted sites (two "Logan restaurants" turned out to be a gambling page and a domain-sale listing). Validation caught both on the absolute price rule rather than publishing them, but nothing should be added without a verified website.
- **National chains are out of reach for the pipeline.** All 30 of the top US chains were checked: 7 return 403 to any bot, 2 time out, and the rest serve a JS shell whose menu is fetched per store, so there is no menu in the html. Taco Bell's 600-odd `$0.00` strings are cart placeholders, and the chains advertising `MenuItem` markup only carry a `BreadcrumbList`. Chains need manual entry.
